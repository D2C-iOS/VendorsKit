...
- ( NSMutableDictionary *)sendPay_demo
{
    ...
    [packageParams setObject: [Wechat notifyUrl]        forKey:@"notify_url"];  //支付结果异步通知
...
- ( NSMutableDictionary *)sendPay_demo_Out_trade_no:(NSString *)out_trade_no name:(NSString *)order_name order_price:(NSInteger)order_price attach:(NSString *)attach   {
    //    //订单标题，展示给用户
    //    NSString *order_name    = @"支付测试~";
    //    //订单金额,单位（分）
    //    NSString *order_price   = @"1";//
    
    NSString *price = [NSString stringWithFormat:@"%ld", order_price];
    
    
//    NSLog(@"   %ld", [order_name length]);
    
    NSString *body = order_name;
    if ([body length] > 32) {
        body = [body substringToIndex:32];
    }
//    NSLog(@"   %ld", [body length]);
